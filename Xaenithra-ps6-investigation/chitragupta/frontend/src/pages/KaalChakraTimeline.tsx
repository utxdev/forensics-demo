...
// --- DATA FETCHING ---
// --- DATA FETCHING ---
const safeMap = (response: any) => {
    if (Array.isArray(response)) return response;
    if (response && Array.isArray(response.data)) return response.data;
    if (response && Array.isArray(response.sms)) return response.sms;
    if (response && Array.isArray(response.calls)) return response.calls;
    if (response && Array.isArray(response.locations)) return response.locations;
    if (response && Array.isArray(response.media)) return response.media;
    return [];
};
...
const fetchIndrajaalData = async (): Promise<ForensicEvent[]> => {
    try {
        ...
        const results = await Promise.all(
            endpoints.map(ep =>
                fetch(`${config.INDRJAAL_API}/api/data/${ep}`)
                    .then(r => r.json())
                    .catch(err => {
        ...
        // ... [rest of function]
        // Skipping to image replacement to do it in one go if possible, or I should do multi_replace.
        // multi_replace is better here.

        const [rawSms, rawCalls, rawLocs, rawMedia] = results;

        const sms = safeMap(rawSms);
        const calls = safeMap(rawCalls);
        const locs = safeMap(rawLocs);
        const media = safeMap(rawMedia);
        ...
        sms.forEach((m: any, i: number) => {
            events.push({
                id: `sms-${i}`,
                type: 'SMS',
                time: m.date ? new Date(m.date).toISOString() : new Date().toISOString(),
                title: m.type === '1' ? 'SMS Received' : 'SMS Sent',
                details: m.body || 'No Content',
        ...
        calls.forEach((c: any, i: number) => {
            // Handle MS timestamps if present
            let timeStr = new Date().toISOString();
            if (c.date) {
                const numDate = Number(c.date);
                timeStr = !isNaN(numDate) ? new Date(numDate).toISOString() : new Date(c.date).toISOString();
            }
            ...
            events.push({
                ...
                type: 'CALL',
                time: timeStr,
                title: `${c.type_label || 'Unknown'} Call`,
        ...
        media.forEach((m: any, i: number) => {
            const time = m.mtime ? new Date(m.mtime * 1000).toISOString() : new Date().toISOString();
            events.push({
                id: `media-${i}`,
                type: 'PHOTO',
                time: time,
                title: 'Media Created',
                details: m.name,
                meta: { size: m.size, path: m.path }
            });
        });

        // If no real data, use mock data as fallback for demo purposes
        if (events.length === 0) {
            console.log("No live data found, using mock data for visualization.");
            return mockIndrajaalData;
        }

        // Sort by time (Newest First usually better for lists, but Oldest First for timeline flow?)
        // Let's do Oldest -> Newest
        events.sort((a, b) => new Date(a.time).getTime() - new Date(b.time).getTime());

        return events;

    } catch (e) {
        console.warn("Critical Failure fetching from Indrajaal Backend.", e);
        return mockIndrajaalData;
    }
};
                        ...
                        <div className="text-center z-10 w-full h-full p-4 flex flex-col items-center justify-center">
                            <div className="w-full h-full bg-gray-900 border border-gray-700 flex items-center justify-center relative">
                                ...
                                {event.details.startsWith('mock') ?
                                    ...
                                    <img src={`${config.INDRJAAL_API}/api/media_content/${event.details}`} alt="evidence" className="max-h-full max-w-full object-contain" />
                                }
