...
const SudarshanaDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (extractionStatus.status === 'running' || extractionStatus.status === 'starting' || extractionStatus.status === 'extracting') {
            interval = setInterval(async () => {
                try {
                    const res = await fetch(`${config.SUDARSHANA_API}/api/sudarshana/extraction_status`);
    ...
    const handleExtraction = async () => {
        try {
            const res = await fetch(`${config.SUDARSHANA_API}/api/sudarshana/extract_data`, { method: 'POST' });
            const data = await res.json();
            if (data.success) {
    ...
    const handleScan = async () => {
        ...
        setScanResult(null);
        try {
            const res = await fetch(`${config.SUDARSHANA_API}/api/sudarshana/scan`, {
                method: 'POST',
    ...
    useEffect(() => {
        const ws = new WebSocket(config.SUDARSHANA_WS);
    ...
    return (
        <div className="h-screen w-full p-4 grid grid-cols-12 gap-4 bg-[#050A18] text-white font-sans overflow-hidden">
            ...
            {/* Header */}
            <div className="col-span-12 flex justify-between items-center border-b border-white/10 pb-4 mb-4 relative z-10">
                <h1 className="text-4xl text-yellow-400 font-display glow-text">SUDARSHANA <span className="text-sm font-sans text-cyan-400 tracking-widest ml-4">THREAT DEFENSE MATRIX</span></h1>
            ...
            {/* Main Radar Area */}
            <div className="col-span-12 lg:col-span-5 flex flex-col items-center justify-start h-[500px] max-h-[500px] relative overflow-hidden p-8 pt-16 border border-white/5 bg-white/5 rounded-xl">
            ...
            <div className="col-span-12 lg:col-span-7 flex flex-col gap-4 h-[500px] max-h-[500px]">
                ...
                <div className="flex-1 border border-white/5 bg-white/5 rounded-xl p-4 flex flex-col overflow-hidden">
