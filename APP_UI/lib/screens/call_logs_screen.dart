...
class _CallLogScreenState extends State<CallLogScreen> {
  ...
  final Map<String, bool> _spamStatus = {};
  ...
  Icon _getIcon(String? type, {double size = 24, Color? color}) {
    if (type == 'INCOMING') {
      return Icon(
        Icons.call_received,
    ...
    if (type == 'OUTGOING') {
      return Icon(Icons.call_made, color: color ?? Colors.blue, size: size);
    }
    if (type == 'MISSED') {
      return Icon(Icons.call_missed, color: color ?? Colors.red, size: size);
    }
    return Icon(Icons.call, size: size, color: color);
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Container(
          ...
          child: Row(
            children: [
              _getIcon(log['type'], size: 28, color: Colors.white),
              const SizedBox(width: 8),
              const Text(
                'Call Details',
